    ...
    await api.get(`date/${date}`)
      .then((response) => {
        if (response.data.data.length > 0) {
          ...
          setMessageSearch(`Ciclos encontrados: ${response.data.data.length}`);
        } else {
          setMessageSearch(`Ciclos encontrados: 0`);
          console.log('Não foram encontrados dados para esta data');
        }
    ...
    <Fragment>
      ...
      <ABoxAll>
        ...
        <div className="flex items-center justify-between mb-2">
          <div>
            <SAddButtom onClick='/ciclos/addciclo' />
          </div>

          {messageSearch != '' &&
            <div className="bg-blue-300 py-0.5 px-4 rounded-lg border border-blue-400 text-blue-900 font-bold">
              <button onClick={() => handleReloadCiclos()} className="flex items-center justify-center">
                <IconContext.Provider value={{ className: "text-2xl" }}>
                  <div className="bg-gray-100 rounded-full p-1 shadow-md border border-white">
                    <IoArrowBack />
                  </div>
                </IconContext.Provider>
                <div className="ml-2">{messageSearch}</div>
              </button>
